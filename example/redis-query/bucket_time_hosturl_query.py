...
print r.zrevrange('hosturls',0,0, withscores=True)
max = int(r.zrevrange('hosturls',0,0, withscores=True)[0][1])

for each in range(0, max, BUCKET_SIZE):
  print "%s: %s" % (each, len(r.zrangebyscore('hosturls',each,each+BUCKET_SIZE, withscores=True)))
